...
ClassAttack.doAttack = function (unit) {
	...
	const data = {
		...
		jab: {
			...
			use: function () {
				return (this.level > 0 && Item.getEquippedItem(4).tier >= 1000);
			}
		},
	};
	...
	if (data.plagueJav.use() && !Attack.checkResist(unit, "lightning")) {
		if (unit.distance <= 15 && !checkCollision(me, unit, 0x4)) {
			...
			!unit.getState(sdk.states.SlowMissiles) && data.slowMissiles.use() && Skill.cast(sdk.skills.SlowMissiles, 0, unit);
	...
	if (data.plagueJav.use() && Attack.checkResist(unit, "poison") && !unit.getState(sdk.states.Poison) && !me.skillDelay) {
		if (((data.plagueJav.force || unit.distance >= 8) && unit.distance <= 25) && !checkCollision(me, unit, 0x4)) {
			Skill.cast(sdk.skills.PlagueJavelin, Skill.getHand(sdk.skills.PlagueJavelin), unit);
		}
	}

	if (data.lightFury.use()) {
		if ((unit.distance >= 8 && unit.distance <= 25) && !checkCollision(me, unit, 0x4)) {
			if (Skill.cast(sdk.skills.LightningFury, Skill.getHand(sdk.skills.LightningFury), unit) && data.lightFury.force) return 1;
		}
	}
	...
	if (result === 2 && Config.TeleStomp && Config.UseMerc && Pather.canTeleport() && Attack.checkResist(unit, "physical") && !!me.getMerc() && Attack.validSpot(unit.x, unit.y)) {
		...
		while (unit.attackable) {
			...
			if (!!closeMob) {
				...
				(this.doCast(closeMob, findSkill.timed, findSkill.untimed) === 1) || (data.decoy.level && Skill.cast(sdk.skills.Decoy, 0, unit));
			}
		}
