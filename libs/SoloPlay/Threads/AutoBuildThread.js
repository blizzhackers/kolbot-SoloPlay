...
function canSpendPoints () {
	let unusedStatPoints = me.getStat(StatPts);
	let haveUnusedStatpoints = unusedStatPoints >= 5;	// We spend 5 stat points per level up
	let unusedSkillPoints = me.getStat(NewSkills);
...
function spendStatPoint (id) {
	...
	return (unusedStatPoints - me.getStat(StatPts) === 1);	// Check if we spent one point
}
...
function spendSkillPoint (id) {
	...
	return (unusedSkillPoints - me.getStat(NewSkills) === 1);	// Check if we spent one point
}
